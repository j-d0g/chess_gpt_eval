...
class StockfishProcessor:
    ...
    def calculate_comprehensive_game_statistics(self, moves_analysis: List[Dict], game_data: Dict, 
        ...
        try:
            ...
            # Add ALL original game data to preserve indexability
            # First add our calculated stats
            final_stats = stats.copy()
            
            # Then add all original game data (this preserves player names, times, etc.)
            for key, value in game_data.items():
                if key not in ['transcript']:  # Skip transcript as it's already processed
                    final_stats[f'original_{key}'] = value
...
def load_csv_data(csv_file: str) -> List[Dict]:
    ...
    try:
        ...
        # Convert to list of dictionaries, preserving ALL original columns
        games = []
        for idx, row in df.iterrows():
            # Use the original game_id if available, otherwise create one
            game_id = row.get('game_id', f"{Path(csv_file).stem}_{idx}")
            
            # Convert row to dictionary, preserving all columns
            game_data = row.to_dict()
            
            # Ensure we have the required fields
            game_data['game_id'] = game_id
            game_data['transcript'] = row.get('transcript', '')
            game_data['result'] = row.get('result', 'unknown')
            game_data['illegal_moves'] = row.get('illegal_moves', 0)
            game_data['number_of_moves'] = row.get('number_of_moves', 0)
            
            # Add source file information
            game_data['source_file'] = Path(csv_file).name
    ...
    for result in results:
        if 'error' in result:
            ...
            summary_row = {
                ...
                **stats  # This now includes all original_ prefixed columns
            }
